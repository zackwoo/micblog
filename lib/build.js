...
var buildIndex = function () {
    ...
    fs.writeFile(wp.join(wp.releaseDir, 'index.html'), htmlMinify(index));
};
//创建文章
var buildArticles = function () {
    var createArticles = function (mdFilePath, articleCfg) {
        fs.readFile(mdFilePath, 'utf-8', function (err, data) {
            ...
            fs.writeFile(wp.join(wp.releaseDir, 'articles/', articleCfg.id + ".html"), htmlMinify(article));
            console.log("create " + articleCfg.id + ".html");
        });
    };
    eachMDfiles(function (file, fileName, fileConfig) {
...
var buildTag = function () {
    ...
    fs.writeFile(wp.join(wp.releaseDir, "tags.html"), htmlMinify(tags));
}
