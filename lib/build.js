...
var buildIndex = function() {
    var indexFileName = function(currentIndex){
        if (currentIndex) {
            return "index_"+currentIndex+".html";
        } else{
            return "index.html";
        }
    };
    if (context.site.paging.enabled) {
        var articles = context.site.articles;
        var paging = context.site.paging;
        for (var i = 0; i < paging.count; i++) {
            context.site.articles = articles.slice(i*paging.size,(i+1)*paging.size);
            paging.disabledPrev = i===0;
            paging.disabledNext = i===(paging.count-1);
            if (!paging.disabledPrev) {
                paging.previous = indexFileName(i-1);
            }
            if (!paging.disabledNext) {
                paging.next = indexFileName(i+1);
            }
            var index = template["index"](context.site);
            fs.writeFile(wp.join(wp.releaseDir, indexFileName(i)), _htmlMinify(index), function(err) {
                if (err) throw err;
            });
        }
    } else {
